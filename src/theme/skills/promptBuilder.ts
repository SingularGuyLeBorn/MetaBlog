...
export function buildSystemPrompt(
  ...
  parts.push([
    ...
    '7. **禁止直接调用不熟悉的领域工具**：看到工具描述不等于知道正确的使用方式. 涉及飞书文档排版(Markdown/LaTeX 格式约束)、GitHub 工作流、学术论文格式等复杂领域时,**必须先加载对应 Skill**(如 feishu-assistant、github-assistant、academic-research),获得完整工作流指导后再调用工具. 直接调用而不加载 Skill 会导致参数格式错误、约束遗漏、调用失败. ',
    '',
    '## 核心原则',
    '',
    '1. **理解优先**: 充分理解用户需求后再行动',
