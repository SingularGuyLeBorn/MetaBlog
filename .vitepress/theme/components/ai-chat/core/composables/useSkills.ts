/**
 * useSkills - Skill 管理系统
 * 
 * 功能：
 * 1. 从文件系统加载 skills（.md 文件）
 * 2. 支持用户上传/创建/编辑/删除 skills
 * 3. 运行时动态加载 skill
 */
import { ref, computed } from 'vue'

/** Skill 分类 */
export type SkillCategory = 'content' | 'development' | 'analysis' | 'language' | 'editing' | 'education' | 'creativity' | 'custom'

export interface Skill {
  id: string
  name: string
  description: string
  icon: string
  category: SkillCategory
  systemPrompt: string
  version: string
  author?: string
  createdAt?: string
  updatedAt?: string
  tags?: string[]
  filePath?: string
}

export interface SkillCreateParams {
  name: string
  description: string
  icon?: string
  category?: string
  systemPrompt: string
  tags?: string[]
}

// Skill 存储目录
const SKILLS_DIR = '/.skills'

// 内置技能（默认）
const BUILTIN_SKILLS: Skill[] = [
  {
    id: 'write',
    name: '写作助手',
    description: '基于提示词生成文章',
    icon: '✍️',
    category: 'content',
    systemPrompt: '你是一个专业的写作助手，擅长根据用户需求创作高质量文章。\n\n## 写作原则\n1. 结构清晰，逻辑连贯\n2. 语言流畅，表达准确\n3. 根据主题选择合适的写作风格\n4. 适当使用标题、列表等格式化元素',
    version: '1.0.0',
    tags: ['写作', '内容创作']
  },
  {
    id: 'summarize',
    name: '文章总结',
    description: '总结文章内容，提取要点',
    icon: '📋',
    category: 'analysis',
    systemPrompt: '你是一个总结专家，擅长提炼文章核心观点和关键信息。\n\n## 总结原则\n1. 提取核心观点和关键信息\n2. 保持客观，不添加个人观点\n3. 结构清晰，层次分明\n4. 适当保留重要细节和数据',
    version: '1.0.0',
    tags: ['总结', '分析']
  },
  {
    id: 'translate',
    name: '中英翻译',
    description: '将内容翻译为英文或中文',
    icon: '🌐',
    category: 'language',
    systemPrompt: '你是一个专业翻译，能够准确流畅地进行中英互译。\n\n## 翻译原则\n1. 准确传达原文意思\n2. 符合目标语言的习惯表达\n3. 保持原文的语气和风格\n4. 专业术语翻译准确',
    version: '1.0.0',
    tags: ['翻译', '语言']
  },
  {
    id: 'polish',
    name: '润色优化',
    description: '优化文章表达，提升可读性',
    icon: '✨',
    category: 'editing',
    systemPrompt: '你是一个文字润色专家，擅长优化表达、修正语法错误。\n\n## 润色原则\n1. 修正语法和拼写错误\n2. 优化句子结构，提升流畅度\n3. 改进用词，增强表达力\n4. 保持原文的核心意思和风格',
    version: '1.0.0',
    tags: ['润色', '编辑']
  },
  {
    id: 'code',
    name: '代码生成',
    description: '生成代码示例和解释',
    icon: '💻',
    category: 'development',
    systemPrompt: '你是一个编程专家，能够编写清晰、高效的代码并提供详细解释。\n\n## 编程原则\n1. 编写简洁、可读性强的代码\n2. 添加必要的注释说明\n3. 考虑边界情况和错误处理\n4. 提供代码解释和使用示例',
    version: '1.0.0',
    tags: ['编程', '代码']
  },
  {
    id: 'review',
    name: '代码审查',
    description: '审查代码，发现潜在问题',
    icon: '🔍',
    category: 'development',
    systemPrompt: '你是一个经验丰富的代码审查员，擅长发现代码中的问题和改进点。\n\n## 审查维度\n1. 代码风格和规范\n2. 潜在的 bug 和错误\n3. 性能优化建议\n4. 安全漏洞检查\n5. 可维护性评估',
    version: '1.0.0',
    tags: ['代码审查', '质量']
  },
  {
    id: 'explain',
    name: '概念解释',
    description: '解释复杂概念，通俗易懂',
    icon: '💡',
    category: 'education',
    systemPrompt: '你是一个优秀的教育者，擅长将复杂概念解释得通俗易懂。\n\n## 解释原则\n1. 使用类比和例子帮助理解\n2. 从简单到复杂，循序渐进\n3. 避免过多专业术语，必要时解释\n4. 鼓励提问和互动',
    version: '1.0.0',
    tags: ['教育', '解释']
  },
  {
    id: 'brainstorm',
    name: '头脑风暴',
    description: '生成创意点子和解决方案',
    icon: '🧠',
    category: 'creativity',
    systemPrompt: '你是一个创意专家，擅长头脑风暴和生成创新点子。\n\n## 创意原则\n1. 数量优先，先产生大量点子\n2. 鼓励 unconventional 的想法\n3. 从不同角度思考问题\n4. 结合点子，产生新的组合',
    version: '1.0.0',
    tags: ['创意', '头脑风暴']
  }
]

// ==================== State ====================
const skills = ref<Skill[]>([...BUILTIN_SKILLS])
const isLoading = ref(false)
const error = ref<string | null>(null)
const categories = computed(() => {
  const cats = new Set(skills.value.map(s => s.category).filter(Boolean))
  return Array.from(cats).sort()
})

// ==================== API Functions ====================

/**
 * 从文件系统加载所有 skills
 */
async function loadSkillsFromFilesystem(): Promise<Skill[]> {
  const loadedSkills: Skill[] = []
  
  try {
    // 1. 获取 skills 目录下的所有 .md 文件
    const response = await fetch(`/api/files/list?path=${encodeURIComponent('.skills')}`)
    if (!response.ok) {
      // 目录可能不存在，返回空数组
      return loadedSkills
    }
    
    // 检查 Content-Type 确保是 JSON
    const contentType = response.headers.get('content-type')
    if (!contentType || !contentType.includes('application/json')) {
      // API 可能不存在，静默返回空数组
      return loadedSkills
    }
    
    const result = await response.json()
    if (!result.success || !Array.isArray(result.data)) {
      return loadedSkills
    }
    
    // 2. 读取每个 .md 文件
    const mdFiles = result.data.filter((f: any) => 
      f.type === 'file' && f.name.endsWith('.md')
    )
    
    for (const file of mdFiles) {
      try {
        const skill = await parseSkillFile(file.name)
        if (skill) {
          loadedSkills.push(skill)
        }
      } catch (e) {
        console.error(`[useSkills] Failed to parse skill file: ${file.name}`, e)
      }
    }
  } catch (e) {
    console.error('[useSkills] Failed to load skills from filesystem:', e)
  }
  
  return loadedSkills
}

/**
 * 解析 skill markdown 文件
 */
async function parseSkillFile(filename: string): Promise<Skill | null> {
  try {
    const response = await fetch(`/api/files/read?path=${encodeURIComponent('.skills/' + filename)}`)
    if (!response.ok) return null
    
    const content = await response.text()
    
    // 解析 frontmatter
    const frontmatterMatch = content.match(/^---\n([\s\S]*?)\n---/)
    if (!frontmatterMatch) return null
    
    const frontmatter = frontmatterMatch[1]
    const systemPrompt = content.replace(/^---\n[\s\S]*?\n---\n*/, '').trim()
    
    // 解析 frontmatter 字段
    const parseField = (name: string, defaultValue: string = ''): string => {
      const match = frontmatter.match(new RegExp(`^${name}:\\s*(.+)$`, 'm'))
      return match ? match[1].trim().replace(/^["']|["']$/g, '') : defaultValue
    }
    
    const parseArray = (name: string): string[] => {
      const match = frontmatter.match(new RegExp(`^${name}:\\s*\\[(.+?)\\]`, 'm'))
      if (!match) return []
      return match[1].split(',').map(s => s.trim().replace(/^["']|["']$/g, ''))
    }
    
    const id = filename.replace('.md', '')
    
    return {
      id,
      name: parseField('name', id),
      description: parseField('description'),
      icon: parseField('icon', '🔧'),
      category: parseField('category', 'custom') as SkillCategory,
      systemPrompt,
      version: parseField('version', '1.0.0'),
      author: parseField('author'),
      createdAt: parseField('createdAt'),
      updatedAt: parseField('updatedAt'),
      tags: parseArray('tags'),
      filePath: '.skills/' + filename
    }
  } catch (e) {
    console.error('[useSkills] Parse error:', e)
    return null
  }
}

/**
 * 创建新的 skill
 */
async function createSkill(params: SkillCreateParams): Promise<Skill | null> {
  try {
    const id = params.name
      .toLowerCase()
      .replace(/[^\w\s]/g, '')
      .replace(/\s+/g, '-')
      .substring(0, 50)
    
    const now = new Date().toISOString()
    
    const frontmatter = `---
name: "${params.name}"
description: "${params.description}"
icon: "${params.icon || '🔧'}"
category: "${params.category || 'custom'}"
version: "1.0.0"
createdAt: "${now}"
updatedAt: "${now}"
tags: [${(params.tags || []).map(t => `"${t}"`).join(', ')}]
---

${params.systemPrompt}`
    
    // 保存到文件系统
    const response = await fetch('/api/files/save', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        path: `.skills/${id}.md`,
        content: frontmatter,
        overwrite: false
      })
    })
    
    if (!response.ok) {
      const error = await response.text()
      throw new Error(error)
    }
    
    const skill: Skill = {
      id,
      name: params.name,
      description: params.description,
      icon: params.icon || '🔧',
      category: (params.category || 'custom') as SkillCategory,
      systemPrompt: params.systemPrompt,
      version: '1.0.0',
      createdAt: now,
      updatedAt: now,
      tags: params.tags || [],
      filePath: `.skills/${id}.md`
    }
    
    // 添加到列表
    skills.value.push(skill)
    
    return skill
  } catch (e) {
    console.error('[useSkills] Failed to create skill:', e)
    return null
  }
}

/**
 * 更新 skill
 */
async function updateSkill(id: string, updates: Partial<Skill>): Promise<boolean> {
  try {
    const index = skills.value.findIndex(s => s.id === id)
    if (index === -1) return false
    
    const skill = skills.value[index]
    
    // 不允许修改内置 skill
    if (!skill.filePath && BUILTIN_SKILLS.some(s => s.id === id)) {
      throw new Error('Cannot modify built-in skills')
    }
    
    const updatedSkill = {
      ...skill,
      ...updates,
      updatedAt: new Date().toISOString()
    }
    
    // 重新生成文件内容
    const frontmatter = `---
name: "${updatedSkill.name}"
description: "${updatedSkill.description}"
icon: "${updatedSkill.icon}"
category: "${updatedSkill.category}"
version: "${updatedSkill.version}"
author: "${updatedSkill.author || ''}"
createdAt: "${updatedSkill.createdAt || ''}"
updatedAt: "${updatedSkill.updatedAt}"
tags: [${(updatedSkill.tags || []).map(t => `"${t}"`).join(', ')}]
---

${updatedSkill.systemPrompt}`
    
    const response = await fetch('/api/files/save', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        path: updatedSkill.filePath || `.skills/${id}.md`,
        content: frontmatter,
        overwrite: true
      })
    })
    
    if (!response.ok) {
      throw new Error('Failed to save skill file')
    }
    
    skills.value[index] = updatedSkill
    return true
  } catch (e) {
    console.error('[useSkills] Failed to update skill:', e)
    return false
  }
}

/**
 * 删除 skill
 */
async function deleteSkill(id: string): Promise<boolean> {
  try {
    const skill = skills.value.find(s => s.id === id)
    if (!skill) return false
    
    // 不允许删除内置 skill
    if (!skill.filePath && BUILTIN_SKILLS.some(s => s.id === id)) {
      throw new Error('Cannot delete built-in skills')
    }
    
    // 删除文件
    const response = await fetch('/api/files/delete', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        path: skill.filePath || `.skills/${id}.md`
      })
    })
    
    if (!response.ok) {
      throw new Error('Failed to delete skill file')
    }
    
    // 从列表移除
    skills.value = skills.value.filter(s => s.id !== id)
    return true
  } catch (e) {
    console.error('[useSkills] Failed to delete skill:', e)
    return false
  }
}

/**
 * 导入 skill 文件
 */
async function importSkillFile(file: File): Promise<Skill | null> {
  try {
    const content = await file.text()
    
    // 验证格式
    if (!content.match(/^---\n[\s\S]*?\n---/)) {
      throw new Error('Invalid skill file format: missing frontmatter')
    }
    
    // 生成文件名
    const id = file.name.replace('.md', '')
    
    // 上传文件
    const response = await fetch('/api/files/save', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        path: `.skills/${id}.md`,
        content,
        overwrite: false
      })
    })
    
    if (!response.ok) {
      const error = await response.text()
      throw new Error(error)
    }
    
    // 解析并返回
    const skill = await parseSkillFile(`${id}.md`)
    if (skill) {
      // 检查是否已存在
      const existingIndex = skills.value.findIndex(s => s.id === skill.id)
      if (existingIndex >= 0) {
        skills.value[existingIndex] = skill
      } else {
        skills.value.push(skill)
      }
    }
    
    return skill
  } catch (e) {
    console.error('[useSkills] Failed to import skill:', e)
    return null
  }
}

/**
 * 导出 skill 为文件
 */
function exportSkill(skill: Skill): string {
  const frontmatter = `---
name: "${skill.name}"
description: "${skill.description}"
icon: "${skill.icon}"
category: "${skill.category}"
version: "${skill.version}"
author: "${skill.author || ''}"
createdAt: "${skill.createdAt || ''}"
updatedAt: "${skill.updatedAt || ''}"
tags: [${(skill.tags || []).map(t => `"${t}"`).join(', ')}]
---

${skill.systemPrompt}`
  
  return frontmatter
}

/**
 * 初始化：加载所有 skills
 */
async function initSkills() {
  isLoading.value = true
  error.value = null
  
  try {
    // 先确保 skills 目录存在
    await fetch('/api/files/mkdir', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ path: '.skills' })
    }).catch(() => {/* 目录可能已存在 */})
    
    // 加载文件系统的 skills
    const fileSkills = await loadSkillsFromFilesystem()
    
    // 合并：内置 skills + 文件 skills（文件 skills 优先级更高）
    const builtinIds = new Set(BUILTIN_SKILLS.map(s => s.id))
    const customSkills = fileSkills.filter(s => !builtinIds.has(s.id))
    
    skills.value = [...BUILTIN_SKILLS, ...customSkills]
  } catch (e) {
    error.value = 'Failed to load skills'
    console.error(e)
  } finally {
    isLoading.value = false
  }
}

// ==================== Composable Export ====================
export function useSkills() {
  return {
    // State
    skills,
    isLoading,
    error,
    categories,
    
    // Getters
    builtinSkills: computed(() => skills.value.filter(s => !s.filePath)),
    customSkills: computed(() => skills.value.filter(s => s.filePath)),
    getSkillById: (id: string) => skills.value.find(s => s.id === id),
    getSkillsByCategory: (category: string) => 
      skills.value.filter(s => s.category === category),
    
    // Actions
    initSkills,
    createSkill,
    updateSkill,
    deleteSkill,
    importSkillFile,
    exportSkill,
    refreshSkills: loadSkillsFromFilesystem
  }
}
